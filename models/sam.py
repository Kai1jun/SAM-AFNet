...
from .mmseg.models.sam import ImageEncoderViT, MaskDecoder, TwoWayTransformer, Prompt3dEncoder
...
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None):
        ...
        # by Kaijun
        # 3D feature prompt encoder
        self.prompt_3d_encoder = Prompt3dEncoder(
            image_embedding_size=[inp_size,inp_size],
            prompt_embed_dim=encoder_mode['prompt_embed_dim'],
            voxel_size=encoder_mode['voxel_size'],
            mask_in_chans=encoder_mode['mask_in_chans']
        )
    ...
    def set_input(self, input, gt_mask, labels):
        self.input = input.to(self.device)
        self.gt_mask = gt_mask.to(self.device)
        # by Kaijun
        # self.pointclouds = pointclouds.to(self.device)
    ...
    def forward(self):
        ...
        # )
        # print('sparse_embeddings1',sparse_embeddings.shape)  # torch.Size([1, 0, 256])
        # print('dense_embeddings1',dense_embeddings.shape)   # torch.Size([1, 256, 64, 64])

        # 从 Prompt3dEncoder 获取特征
        sparse_embeddings, dense_embeddings = self.prompt_3d_encoder(bs, self.labels)
        ...
        # print('self.input', self.input.shape)  # torch.Size([1, 3, 1024, 1024]
