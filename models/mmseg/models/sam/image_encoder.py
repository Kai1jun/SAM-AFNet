...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        inp = x
        # print('before patch_embed', x.shape)  # torch.Size([1, 3, 1024, 1024]
        x = self.patch_embed(x)
        # print('after patch_embed', x.shape)  # torch.Size([1, 64, 64, 1280]

        embedding_feature = self.prompt_generator.init_embeddings(x)
        handcrafted_feature = self.prompt_generator.init_handcrafted(inp)
        # print('embedding_feature:',embedding_feature.shape) # torch.Size([1, 4096, 40]
        # print('handcrafted_feature:',handcrafted_feature.shape) # torch.Size([1, 40, 64, 64]
...
class PromptGenerator(nn.Module):
    ...
    def init_embeddings(self, x):
        # print('before init_embeddings', x.shape)  # torch.Size([1, 64, 64, 1280]
        N, C, H, W = x.permute(0, 3, 1, 2).shape
        x = x.reshape(N, C, H*W).permute(0, 2, 1)
        # print('2 before init_embeddings', x.shape)  # torch.Size([1, 4096, 1280]
        # print(3, self.embedding_generator(x).shape) # torch.Size([1, 4096, 40]
        return self.embedding_generator(x)   # 4-2 Linear

    def init_handcrafted(self, x):
        # print('before init_handcrafted', x.shape)  # torch.Size([1, 3, 1024, 1024]
        x = self.fft(x, self.freq_nums)
        # print(2,x.shape)  # torch.Size([1, 3, 1024, 1024]
        # print('after init_handcrafted', self.prompt_generator(x).shape)  # torch.Size([1, 40, 64, 64])
        return self.prompt_generator(x)

    def get_prompt(self, handcrafted_feature, embedding_feature):
        N, C, H, W = handcrafted_feature.shape
        # handcrafted_feature   torch.Size([1, 40, 64, 64])
        handcrafted_feature = handcrafted_feature.view(N, C, H*W).permute(0, 2, 1)
        # handcrafted_feature   torch.Size([1, 4096, 40])
        prompts = []
        for i in range(self.depth):
            ...
            # print('prompt.shape',prompt.shape)  # torch.Size([1, 4096, 40])
    ...
    def forward(self, x):
        ...
        # get prompting
        print('1,before prompt_generator', x.shape)
        prompt = self.prompt_generator(x)
        print('2,after prompt_generator', prompt.shape)

        if self.mode == 'input':
            print('self.mode == input')
...
class PatchEmbed2(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("1",x.shape)   # torch.Size([1, 3, 1024, 1024])
        x = self.proj(x)
        # print("2",x.shape)  # torch.Size([1, 40, 64, 64])
        return x
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # print("Input shape:", x.shape)   # torch.Size([1, 3, 1024, 1024])
        x = self.proj(x)
        # print("After Conv2d:", x.shape)  # After Conv2d: torch.Size([1, 1280, 64, 64])
        # B C H W -> B H W C
        x = x.permute(0, 2, 3, 1)
        return x


# print network structure code
...
class SAMWrapper(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # print('inputs:',inputs)
        input, gt_mask, labels = inputs
        self.sam.set_input(input, gt_mask, labels)
        return self.sam.forward()
...
if __name__ == '__main__':
    ...
    wrapped_model = SAMWrapper(model)

    # inp = [1, 3, 1024, 1024]  
    # gt = [1, 1, 1024, 1024]
    inp = torch.randn(1, 3, 1024, 1024).cuda()  
    gt = torch.randn(1, 1, 1024, 1024).cuda() 
    labels = torch.randn(1, 1275359, 6).cuda() 
    input_data = (inp, gt, labels)
...
#     base_model = create_model("vox_spikeformer",
#                                 num_classes = 40,
#                                 spike_mode = "if",
#                                 voxel_shape = [48,48,48],
#                                 embed_dims = 256,
#                                 num_heads = 8,
#                                 mlp_ratios = 4,
#                                 depths = 2)
#     print(base_model)
#     if T != 1:
#         functional.set_step_mode( base_model, step_mode='m')
#         functional.set_backend(base_model, 'cupy', neuron.IFNode)
#         print(            
#         str(
#             torchinfo.summary(
#                 base_model, (T, 1, 1024, 32, 32, 32) # T B C W H
#                 )
#             )
#         )
#     else:
#         functional.set_step_mode( base_model, step_mode='s')
#         print(            
#         str(
#             torchinfo.summary(
#                 base_model, (1, 1, 32, 32, 32) # B C W H
#                 )
#             )
#         )
