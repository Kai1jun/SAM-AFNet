...
def train(train_loader, model):
    ...
    for batch in train_loader:
        for k, v in batch.items():
            batch[k] = v.to(device)
        inp = batch['inp']
        # print('inp.shape', inp.shape)  # torch.Size([1, 3, 1024, 1024])
        gt = batch['gt']  
        # print('gt.shape', gt.shape)  # torch.Size([1, 1, 1024, 1024])
        # points = batch['points']
        labels = batch['labels']
        model.set_input(inp, gt, labels)
...
if __name__ == '__main__':
    ...
    save_path = os.path.join('./save/0806', save_name)
